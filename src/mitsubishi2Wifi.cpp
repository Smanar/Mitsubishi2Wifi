...
void setup() {
  ...
  if (SPIFFS.begin())
  {
    write_log(F("Mounted file system"));
  }
  else
  {
  ...
  */
  ...
  if (initWifi())
  {
    ...
    {
      //write_log("Can't load server settings");
    }
...
bool SendJson(const JsonVariant j) {
  ...
  http.begin(espClient, HostURL.c_str());
  http.POST(s);

  //write_log(HostURL.c_str());
  //write_log(s.c_str());
...
bool loadServerSettings() {
  ...
  if (!configFile) {
    //write_log("Failed to open Server config file");
    return false;
  }
  ...
  server_port         = doc["server_Port"].as<String>();
  server_url          = doc["server_url"].as<String>();

  //Update value for application
  HostURL = server_ip + ":" + server_port + server_url;
...
void saveServerSettings(String ip, String url, String server_port) {
  ...
  if (server_port[0] == '\0') server_port = "80";
  if (url[0] == '\0') url = "/";
...
void handleServer() {
  ...
  if (server.method() == HTTP_POST)
  {
    ...
    ServerPage.replace(F("_SERVER_IP_"), server_ip);
    ServerPage.replace(F("_SERVER_PORT_"), String(server_port));
